  ...
  transition: all 1s linear 0s;overflow: hidden;">
                                    ...
                                    <div class="suggestions-tags">
                                        ...
                                        <a href="javascript:void(0);" class="">
                                            <span class="label label-info bgcolor1 badge badge-pill pl-sm-3 pr-sm-3 pr-2 pl-2 text-white mb-2 getSentence" data-id="{{$suggestion->suggesstion_id}}" data-sentence="{{$suggestion->sentence}}" >{{$suggestion->title}}</span>
                                        </a>
        ...
        $.ajax({
                ...
                success: function(data) { 
                    ...
                    if (data['status'] == 'error') {
                        ...
                        $(".carnumber-error").html(data['message']);
                        $('#carnumber-error').show();
                        if(data['captacha']){
                            $("#myForm").show();
                        }
    ...
    function getVersionData(version_id) {
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.door != null) {
                    $('#doors').val(data.door).prop('selected', true);
                }
                if (data.engine_type_id != null) {
                    $('#fuel_type').val(data.engine_type_id).prop('selected', true);
                }
                if (data.transmission_id != null) {
                    $('#transmission_type').val(data.transmission_id).prop('selected', true);
                }
