        ...
        <div class="col-9 mx-auto bg-white border p-sm-4 p-3 py-lg-5 py-4">
          <div class="justify-content-center align-items-center form-group mb-sm-4 mb-3 row">
            <div class="col-md-3 col-lg-2 col-sm-3 mb-1 mb-sm-0 text-sm-right">
              <label class="mb-0 text-capitalize">{{__('dashboardMyProfile.full_name')}}<sup class="text-danger">*</sup></label>
            </div>
            <div class="col-md-6 col-xl-5 col-lg-5 col-sm-7">
              <input type="text" class="form-control" placeholder="{{__('dashboardMyProfile.full_name')}}" id="customer_firstname" name="customer_firstname" value="{{@$customer->customer_company!=null?$customer->customer_company:''}}" required>
          ...
          <div class="justify-content-center align-items-center form-group mb-sm-4 mb-3 row">
            ...
            <div class="col-md-6 col-xl-5 col-lg-5 col-sm-7">
              <select class="form-control" id="city" name="city">
                <option value="">{{__('dashboardMyProfile.select_city')}}</option>
          ...
          <div class="justify-content-center align-items-center form-group mb-sm-4 mb-3 row">
            ...
            <div class="col-2">
              <input type="button" class="btn rounded-0 btn-block" id="contact-verify" value="{{__('dashboardMyProfile.verify')}}" style="padding:.6rem 0rem;" disabled>
            </div>
          </div>
          <!-- Preferred language -->
          <div class="justify-content-center align-items-center form-group mb-sm-4 mb-3 row">
        ...
        <em class="fa fa-globe text-center signuo-field-icon"></em>
            ...
            <div class="col-md-6 col-xl-5 col-lg-5 col-sm-7">
              <select class="form-control" name="prefered_language">
                <option>{{__('dashboardMyProfile.preferredLanguage')}}</option>
          ...
          <div class="justify-content-center align-items-center form-group mb-sm-4 mb-3 row">
            <div class="col-md-6 col-xl-5 col-lg-5 offset-lg-2 offset-sm-3 col-sm-7">
              <input type="submit" class="btn save-btn pl-4 post-ad-submit pr-4  pl-lg-5 pr-lg-5 pt-lg-3 pb-lg-3  themebtn1" value="{{__('dashboardMyProfile.buttonText')}}">
            </div>
          </div>
        </div>
      </div>
            ...
            <form method="post" name="sms_verify_form" id="sms_verify_form" action="{{route('verify-contact-process')}}" target="_blank">
                ...
                <div class="modal-body sms-modal-body">
                    ...
                    <div class="modal-footer" style="justify-content: center;border-top: none;">
                        <button type="button" class="btn themebtn3" id="verify_sms">{{__('dashboardMyProfile.verify')}}</button>
                    </div>
                </div>
            </form>
          </div>
...
$(document).ready(function(){
  $(document).on('submit', '.add-profile-form', function(e){
      ...
      $.ajax({
        ...
        processData: false,
        beforeSend: function() {
          $('.save-btn').val("{{ __('dashboardMyProfile.please_wait') }}...");
  ...
  $("#myForm").validate({
    ...
    messages: {
      customer_firstname: "{{__('dashboardMyProfile.please_enter_firstname')}}",
      customer_lastname: "{{__('dashboardMyProfile.please_enter_lastname')}}",
      gender: "{{__('dashboardMyProfile.please_select_gender')}}",
      dob: "{{__('dashboardMyProfile.please_enter_date_of_birth')}}",
      country: "{{__('dashboardMyProfile.please_select_country')}}",
      city: "{{__('dashboardMyProfile.please_select_city')}}",
      customer_contact: "{{__('dashboardMyProfile.please_enter_phone_number')}}",
      prefered_language: "{{__('dashboardMyProfile.please_select_preferred_language')}}",
  ...
  /*$(document).on('click', '#contact-verify', function() { 
    grecaptcha.ready(function() {
        grecaptcha.execute('{{ env('RECAPTCHA_SITE_KEY') }}', {action: 'contact_verify'}).then(function(token) {
            $("#g-recaptcha-response-individual").val(token);
            verifyContactNumber(token);
        });
    });
  });*/
  ...
  $(document).on('click', '#contact-verify', function() { 
    ...
    // Only proceed with verification if number is actually changed
    if(customer_contact != current_contact) {
        // Check last phone change date first
        $.ajax({
            url: "{{ route('check-last-phone-change') }}",
            type: 'post',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
            },
            success: function(response) {
                if(response.status == 'error') {
                    ...
                    // Proceed with existing verification process
                    grecaptcha.ready(function() {
                        grecaptcha.execute('{{ env('RECAPTCHA_SITE_KEY') }}', {action: 'contact_verify'}).then(function(token) {
                            $("#g-recaptcha-response-individual").val(token);
                            verifyContactNumber(token);
  ...
  function verifyContactNumber(recaptchaToken) {
    ...
    $('.modal-footer').html('<button type="button" class="btn themebtn3" id="verify_sms">{{__('dashboardMyProfile.verify')}}</button>');
    ...
    $.ajax({
              ...
              if(data['status']=='success')
                {
                  ...
                  $('.modal-footer').html('<input type="button" class="btn themebtn3" id="contact-verify" value="{{__('dashboardMyProfile.resend')}}">');
      ...
      if (jqXHR.status === 422) {
        ...
        if (response.status === "error" && response.errors && response.errors.captcha) {
            Swal.fire({
                icon: 'error',
                title: '{{__('signUp.smsPopUpAlertOps')}}',
