         ...
         <aside class="col-lg-3 col-12 mb-4 mb-lg-0 left-sidebar">
            <div class="leftsidebar-bg">
               ...
               <div id="searchResultFilter">
                  ...
                  <!-- Models Section - Initially hidden unless make is selected -->
                  <div class="card">
                     <div class="card-header pl-4 pr-4" id="heading3">
                        <a href="#" class="collapsed align-items-center d-flex" data-toggle="collapse" data-target="#searchby3" aria-expanded="false" aria-controls="searchby3">
                           <figure class="text-center mb-0"><img src="{{url('public/assets/img/rIcon-bodystyle.png')}}" alt="carish used cars for sale in estonia" class="img-fluid"></figure>
                     ...
                     <div id="searchby3" class="collapse" aria-labelledby="heading3" data-parent="#searchResultFilter">
                        <div class="card-body">
                           ...
                           @if(isset($quey_string_slug['mk']))
                              @if(count($side_bar_array['models']) > 0)
                              <ul class="list-unstyled carselectlist mb-0">
                                 @foreach($side_bar_array['models'] as $model)
                                 @if(in_array($model->make_id, collect($side_bar_array['makes'])->where('title', $quey_string_slug['mk'][0])->pluck('id')->toArray()))
                                 <li class="d-flex justify-content-between">
                                    <div class="custom-control mr-2 custom-checkbox ">
                                       <input type="checkbox" class="custom-control-input search_check_submit" value="mo_{{$model->name}}" id="modelsCheck{{$model->id}}" data-value="mo_{{$model->name}}" @if(isset($quey_string_slug['mo']) && in_array($model->name,$quey_string_slug['mo'])) checked="checked" @endif>
                                       <label class="custom-control-label" for="modelsCheck{{$model->id}}">{{$model->name}}</label>
                                    </div>
                                    <div><span class="badge badge-pill border pb-1 pt-1">{{$model->total_ads_in_models}}</span></div>
                                 </li>
                                 @endif
                                 @endforeach
                              </ul>
            ...
            <div class="row">
               ...
               <div class="col-lg-9 col-md-9 col-sm-9 col-8 listingCar-descCar">
                  <div class="align-items-center d-flex justify-content-between listingCar-title mb-1">
                     <!-- added -->
                      <h5 class="font-weight-normal mb-0"><a target="" href="{{url('car-details/'.$show_result->id)}}" target="">{{@$show_result->maker->title}} {{@$show_result->model->name}} {{@$show_result->year}} 
                        {{@intval($show_result->engine_capacity)}}CC 
                        {{@intval($show_result->engine_power)}}kw 
                        {{$show_result->transmission}}
                      </a></h5> 
