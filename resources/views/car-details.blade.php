...
<div class="internal-page-content mt-4 pt2 pt2 sects-bg">
    ...
    background-position: center;padding: 40%;"></div>
      ...
      <div class="col-md-6 col-12 mt-4 mt-md-0 productDetialCol">
        <div class="bg-white border h-100 overflow-auto position-relative">
          <h6 class="bgcolor1 detail-page-title mb-0 pb-3 pl-md-4 pr-md-4 pl-3 pr-3 pt-3 text-white"  data-version="{{@$ads->versions->id}}" data-varient="{{@$ads->versions->CarVarient->id}}">{{$ads->maker->title}} {{$ads->model->name}} {{@$ads->versions->name}} {{$ads->year}}</h6>
          <div class="seller-desc p-lg-4 p-3 mt-3">
            ...
            <div class="container ">
              <div class="mb-4 ml-auto mr-auto mt-3 mt-lg-4 row sellercarInforow bg-white">
                ...
                <div class="col-6 col-md-4 col-sm-4 d-flex justify-content-center sellercarInfo pl-2 pr-2 pt-lg-3 pb-lg-4 pt-2 pb-3">
                  <div class="text-center">
                    <img src="{{asset('public/assets/img/detail-car-info-6.png')}}" class="img-fluid mb-1" alt="carish used cars for sale in estonia">
                    <span class="d-block">
                      {{@intval($ads->versions->engine_capacity_cc)}} cc
                      @if($ads != null)
                      @if(@$ads->versions->engine_power_kw)
                      / {{@intval($ads->versions->engine_power_kw)}} kw
                      @endif
                      @endif
                    </span>
                  </div>
  ...
  <div class="carinfodetail-section bg-white mt-5 pt-5 pb-5">
    ...
    <tr>
        ...
        <td>{{@intval($ads->versions->CarVarient->length)}}</td>
        
        <td>{{ __('carDetailPage.width') }}</td>
        <td>{{@intval($ads->versions->CarVarient->width)}}</td>
    </tr> 
    <tr>
        <td>{{ __('carDetailPage.height') }}</td>
        <td>{{@intval($ads->versions->CarVarient->height)}}</td>
        
        <td>{{__('carDetailPage.curbWeight')}}</td>
        <td>
          @if($ads->versions->CarVarient->full_mass !=null)
            {{@intval($ads->versions->CarVarient->full_mass)}}
          @endif
        </td>
    </tr>
    ...
    <tr>
      ...
      <td>{{__('carDetailPage.axleCount')}}</td>
      <td>
        @if($ads->versions->CarVarient->total_axle !=null) 
          {{@intval($ads->versions->CarVarient->total_axle)}}
        @endif
      </td>
        
      <td>{{__('carDetailPage.doors')}}</td>
      <td>
        @if($ads->versions->CarVarient->door != null) 
          {{@intval($ads->versions->CarVarient->door)}}
        @endif
      </td>
    </tr>
    ...
    <tr>
        ...
        <td>@if($ads->versions->CarVarient->num_of_seats != null) {{$ads->versions->CarVarient->num_of_seats}} @endif</td>

        <td>{{__('carDetailPage.emissionStandard')}}</td>
        <td>{{!empty($ads->versions->CarVarient->emission_standard) ? @$ads->versions->CarVarient->emission_standard : '--'}}</td>
     
    </tr>

    
    <tr>
        <td>{{__('carDetailPage.noiseLevel')}}</td>
        <td>{{!empty($ads->versions->CarVarient->noise_level) ? @$ads->versions->CarVarient->noise_level : '--'}}</td>

        <td>{{__('carDetailPage.co2Nedc')}}</td>
        <td>{{!empty($ads->versions->CarVarient->co2_nedc) ? @$ads->versions->CarVarient->co2_nedc : '--'}}</td>
     
    </tr>
    <tr>
        <td>{{__('carDetailPage.averageFuelConsuption')}}</td>
        <td>{{!empty($ads->versions->CarVarient->average_fuel_consuption) ? @$ads->versions->CarVarient->average_fuel_consuption : '--'}}</td>

        <td>{{__('carDetailPage.maxSpeed')}}</td>
        <td>{{!empty($ads->versions->CarVarient->maximum_speed) ? @intval($ads->versions->CarVarient->maximum_speed) : '--'}}</td>
    </tr>
