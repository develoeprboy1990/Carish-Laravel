...
class AdsController extends Controller
{
  ...
  public function getModelsByYearVersionsOld($model_id, $year)
  {
    $activeLanguage = \Session::get('language');
    $html = '';
    if (request()->ajax()) {
  ...
  public function getModelsByYearVersions($model_id, $year)
  {
    ...
    if (request()->ajax()) {
          ...
          $versions_query = Version::query()->select(
            'versions.id as version_id',
            'engine_capacity_cc as cc',
            'engine_power_kw as kw',
            'first_registration',
            'bt.name AS body_name',
            't.title AS transmission'
          )->where('model_id', $model_id)->where('first_registration', $year);

          $versions_query->leftJoin('body_types_description AS bt', function ($join) use ($activeLanguage) {
            $join->on('versions.body_type_id', '=', 'bt.body_type_id');
            $join->where('bt.language_id', $activeLanguage['id']);
          });

          $versions_query->leftJoin('transmission_description AS t', function ($join) use ($activeLanguage) {
            $join->on('versions.transmission_id', '=', 't.transmission_id');
            $join->where('t.language_id', $activeLanguage['id']);
          });



            $versions       = $versions_query->orderBy('engine_capacity_cc', "ASC")->orderBy('engine_power_kw', "ASC")->get(); 
            $set_versions = array();
            if (!$versions->isEmpty()) {
                foreach ($versions as $version) {

                    $set_versions[$version['body_name']][] = array(
                        'vearsion_title' => (int)$version['cc'].' CC, '.(int)$version['kw'].'kw, '.$version['transmission'],
                        'cc' => (int)$version['cc'],
                        'kw' => (int)$version['kw'],
                ...
                foreach ($set_versions as $keys => $versions_array) {
                    ...
                    foreach ($versions_array as $versions_values) {
                        $html .= '
                <li class="versions" data-title="'.$versions_values['vearsion_title'].'"  data-version="'.$versions_values['version_id'].'" id="version_' . $versions_values['version_id'] . '" data-cc="' . $versions_values['cc'] . '"  data-power="' . $versions_values['kw'] . '"><a href="javascript:void(0)">' . $versions_values['vearsion_title'] . '</a></li>
                        ';
                    } // END FOREACH INNER
