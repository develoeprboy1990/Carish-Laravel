...
&& strpos($parent_child_cat[1], 'rq_') === false)
  ...
  public function companyProfile($id)
  {
    ...
    $ads       = Ad::join('makes', 'ads.maker_id', '=', 'makes.id')->orderBy('makes.title', 'ASC')
      ->select('ads.*', 'v.engine_power_kw AS engine_power', 'v.engine_capacity_cc as engine_capacity')
  ...
  public function getAllVersions()
  {
    if (request()->ajax()) {
      ...
      $term   = request()->get('q');
      $models = array();
      $versions = Version::select('id', 'label', 'engine_capacity_cc', 'engine_power_kw')
        ->where('label', 'like', "%$term%")->get();
      if (!$versions->isEmpty()) {
        foreach ($versions as $version) {
          $models[] = array('id' => "ver_" . str_replace(" / ", "", $version->label), 'text' => $version->label . ' ' . $version->engine_capacity_cc . " CC" . ' ' . $version->engine_power_kw . ' KW');
        }
      }
  ...
  public function getAllVersionsBy($term)
  {
    if (request()->ajax()) {
      ...
      $select_array = array('id', 'label', 'engine_capacity_cc', 'engine_power_kw');
      ...
      if (!$versions->isEmpty()) {
        foreach ($versions as $version) {
          $html .= '<option value="ver_' . str_replace(" / ", "!", $version->label) . "-" . $version->engine_capacity_cc . "-CC-" . $version->engine_power_kw . '-KW' . '">' . $version->label . ' ' . $version->engine_capacity_cc . " CC" . ' ' . $version->engine_power_kw . ' KW' . '</option>';
          /* $models[] = array('id'=>"ver_".$version->label,'text'=>$version->label.' '.$version->cc." cc".' '.$version->engine_power_kw.' KW');*/
        }
      }
      return $html;
    }
  ...
  public function getVersionsCC()
  {
    ...
    if (request()->ajax()) {
      $html = '';
      $versions   = Version::select('engine_capacity_cc', 'id')->distinct('engine_capacity_cc')->groupBy('engine_capacity_cc')->get();
...
*/
      if (!$versions->isEmpty()) {
        foreach ($versions as $version) {
          $html .= '<option value="' . $version->engine_capacity_cc . '"  >' . $version->engine_capacity_cc . ' CC</option>';
        }
      }
      return $html;
    }
  ...
  public function getVersionsPOwer()
  {
    ...
    if (request()->ajax()) {
      $html = '';
      $versions   = Version::select('engine_power_kw', 'id')->distinct('engine_power_kw')->groupBy('engine_power_kw')->get();
      /* 'strict' => false, id db file $logs = DB::table('versions')->select('cc','id')->groupBy('cc')->get();
              $versions = Version::selectRaw('distinct(cc), id')->groupBy('cc')->get();
*/
      if (!$versions->isEmpty()) {
        foreach ($versions as $version) {
          $html .= '<option value="' . $version->engine_power_kw . '"  >' . $version->engine_power_kw . ' KW</option>';
        }
      }
      return $html;
    }
  ...
  public function simpleSearch($params)
  {
    ...
    $select_array    = array(
      'ads.id', 'ads.fuel_type', 'description', 'year', 'millage', 'fuel_average',
      'price', 'vat', 'neg', 'features', 'etd.title AS engine_title', 'gbd.title AS transmission_title', 'assembly',
      'cv.door as doors', 'cv.length', 'cv.height', 'cv.width', 'poster_name', 'poster_email', 'poster_phone',
      'is_featured', 'views', 'ads.status', 'maker_id', 'mo.id as model_id', 'version_id',
      'customer_id', 'm.title as make_name', 'mo.name as model_name',
      'v.engine_capacity_cc as engine_capacity',
      'v.engine_power_kw AS engine_power',
      't.title AS transmission',
      'c.name as city_name', 'ads.updated_at AS last_updated', 'p.img'
    );
    $ads_results->leftJoin('makes AS m', 'm.id', '=', 'ads.maker_id');
    $ads_results->leftJoin('versions AS v', 'v.id', '=', 'ads.version_id');
    $ads_results->leftJoin('car_varients AS cv', 'cv.id', '=', 'v.car_variant_id');
    $ads_results->leftJoin('models AS mo', 'mo.id', '=', 'ads.model_id');
    $ads_results->leftJoin('cities AS c', 'c.id', '=', 'ads.poster_city');
    $ads_results->leftJoin('transmission_description AS t', function ($join) use ($activeLanguage) {
            $join->on('v.transmission_id', '=', 't.transmission_id');
    ...
    if (Arr::has($quey_string_slug, 'ver')) {
      ...
      if (!empty($power)) {
        $ads_results->whereIn('v.engine_capacity_cc', $power);
      }
      if (!empty($enginecc)) {
        $ads_results->whereIn('v.engine_power_kw', $enginecc);
      }
    ...
    if (Arr::has($quey_string_slug, 'power')) {
      ...
      if (count($power) == 1) {
        $ads_results->where('v.engine_power_kw', '>=', $power[0]);
        $query_string_array['power'] = " AND v.engine_power_kw >='" . $power[0] . "'";
      } else if (count($power) == 2 && $power[0] == "") {
        $ads_results->where('v.engine_power_kw', '<=', $power[1]);
        $query_string_array['power'] = " AND v.engine_power_kw <='" . $power[0] . "'";
      } else if (count($power) == 2 && $power[1] == "") {
        $ads_results->where('v.engine_power_kw', '>=', $power[0]);
        $query_string_array['power'] = " AND v.engine_power_kw >='" . $power[0] . "'";
      } else {
        $ads_results->whereBetween('v.engine_power_kw', [$power[0], $power[1]]);
        $query_string_array['power'] = " AND v.engine_power_kw BETWEEN '" . $power[0] . "' AND '" . $power[1] . "'";
      }
    ...
    if (Arr::has($quey_string_slug, 'enginecc')) {
      ...
      if (count($enginecc) == 1) {
        $ads_results->where('v.engine_capacity_cc', '>=', $enginecc[0]);
        $query_string_array['enginecc'] = " AND v.engine_capacity_cc >='" . $enginecc[0] . "'";
      } else if (count($enginecc) == 2 && $enginecc[0] == "") {
        $ads_results->where('v.engine_capacity_cc', '<=', $enginecc[1]);
        $query_string_array['enginecc'] = " AND v.engine_capacity_cc <='" . $enginecc[0] . "'";
      } else if (count($enginecc) == 2 && $enginecc[1] == "") {
        $ads_results->where('v.engine_capacity_cc', '>=', $enginecc[0]);
        $query_string_array['enginecc'] = " AND v.engine_capacity_cc >='" . $enginecc[0] . "'";
      } else {
        $ads_results->whereBetween('v.engine_capacity_cc', [$enginecc[0], $enginecc[1]]);
        $query_string_array['enginecc'] = " AND v.engine_capacity_cc BETWEEN '" . $enginecc[0] . "' AND '" . $enginecc[1] . "'";
      }
    ...
    $whereCondition = "SELECT m.id, m.title, 
    (SELECT COUNT(*) FROM ads a WHERE a.maker_id = m.id AND a.status = 1) as total_ads_in_makes 
    FROM makes m 
    WHERE m.status = 1 
    ORDER BY m.title";
    $side_bar_array['makes'] = $this->getSelect($whereCondition);
    ...
    /*$whereCondition = "SELECT mo.id, mo.name,
    (SELECT COUNT(*) FROM ads a WHERE a.model_id = mo.id AND a.status = 1) as total_ads_in_models 
    FROM models mo 
    WHERE mo.status = 1 
    ORDER BY mo.name";
    $side_bar_array['models'] = $this->getSelect($whereCondition);*/
    ...
    if (Arr::has($quey_string_slug, 'mk')) {
        $whereCondition = "SELECT mo.id, mo.name, mo.make_id,
        (SELECT COUNT(*) FROM ads a WHERE a.model_id = mo.id AND a.status = 1) as total_ads_in_models 
        FROM models mo 
        INNER JOIN makes m ON mo.make_id = m.id
        WHERE mo.status = 1 AND m.title = '" . $quey_string_slug['mk'][0] . "'
        ORDER BY mo.name";
    } else {
        // When no make is selected, return empty array
        $whereCondition = "SELECT mo.id, mo.name, mo.make_id,
        0 as total_ads_in_models 
        FROM models mo 
        WHERE 1=0";
    }
    $side_bar_array['models'] = $this->getSelect($whereCondition);
    ...
    $whereCondition = "SELECT c.id, cd.name,
    (SELECT COUNT(*) FROM ads a WHERE a.color_id = c.id AND a.status = 1) as total_ads_in_color 
    FROM colors c 
    INNER JOIN colors_description cd ON c.id = cd.color_id 
    WHERE c.status = 1 AND cd.language_id = " . $activeLanguage['id'] . "
    ORDER BY cd.name";
    $side_bar_array['colors'] = $this->getSelect($whereCondition);
    ...
    $whereCondition = "SELECT bt.id, btd.name,
    (SELECT COUNT(*) FROM ads a WHERE a.body_type_id = bt.id AND a.status = 1) as total_ads_in_body 
    FROM body_types bt 
    INNER JOIN body_types_description btd ON bt.id = btd.body_type_id 
    WHERE bt.status = 1 AND btd.language_id = " . $activeLanguage['id'] . "
    ORDER BY btd.name";
    $side_bar_array['bodyTypes'] = $this->getSelect($whereCondition);
    ...
    $whereCondition = "SELECT c.id, c.name,
    (SELECT COUNT(*) FROM ads a WHERE a.poster_city = c.id AND a.status = 1) as total_ads_in_cities 
    FROM cities c 
    WHERE c.status = 1 
    ORDER BY c.name";
    $side_bar_array['locations'] = $this->getSelect($whereCondition);
    ...
    $whereCondition = "SELECT t.id, td.title,
    (SELECT COUNT(*) FROM ads a WHERE a.transmission_type = t.id AND a.status = 1) as total_ads_in_transmission 
    FROM transmissions t 
    INNER JOIN transmission_description td ON t.id = td.transmission_id 
    WHERE t.status = 1 AND td.language_id = " . $activeLanguage['id'] . "
    ORDER BY td.title";
    $side_bar_array['transmission'] = $this->getSelect($whereCondition);
    ...
    $whereCondition = "SELECT et.id, etd.title,
    (SELECT COUNT(*) FROM ads a WHERE a.fuel_type = et.id AND a.status = 1) as total_ads_in_fuel 
    FROM engine_types et 
    INNER JOIN engine_types_description etd ON et.id = etd.engine_type_id 
    WHERE et.status = 1 AND etd.language_id = " . $activeLanguage['id'] . "
    ORDER BY etd.title";
    $side_bar_array['fuel'] = $this->getSelect($whereCondition);
  ...
  public function simpleSearch1($params)
  {
    ...
    $select_array = array(
      ...
      'v.label as version_label', 'v.engine_capacity_cc as engine_capacity', 'v.engine_power_kw AS engine_power', 'c.name as city_name', DB::raw('DATE_FORMAT(current_date(), "%d-%m-%Y") - DATE_FORMAT(ads.updated_at, "%d-%m-%Y") as last_updated')
    );
