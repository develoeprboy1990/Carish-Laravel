...
class CustomerController extends Controller
{
    ...
    public function updateProfile(Request $request)
    {
        ...
        // If phone number is being changed, update the timestamp
        if ($request->customer_contact != $customer->customers_telephone) {
            $customer->phone_changed_at = Carbon::now();
        }
    ...
    private function uploadd($base64_string, $prodId)
    {
        ...
        if (empty($part) or @$part[1] == null or empty(@$part[1])) {
            ...
            $img = $manager->read(base64_decode($dataa[1]));
            $img->resize($new_width, $new_height);
            $img->save($pathToImage);
    ...
    public function createService($id = null)
    {
        ...
        return view('users.ads.serviceForm', compact('categorise', 'customer', 'timings', 'primaryServices', 'services', 'service_description', 'page_description','ads_pricing','credit','cities'));
    }
    ...
    public function checkOldPasswordBusiness(Request $request)
{
    $hashedPassword = Auth::guard('customer')->user()->customer_password;
    $old_password = $request->old_password;
    ...
    public function checkLastPhoneChange(Request $request) {
        $customer = Customer::where('id', Auth::guard('customer')->user()->id)->first();
        
        if ($customer->phone_changed_at) {
            $lastChanged = Carbon::parse($customer->phone_changed_at);
            $monthAgo = Carbon::now()->subDays(30);
            
            if ($lastChanged->gt($monthAgo)) {
                $nextChangeDate = $lastChanged->addDays(30)->format('d M Y');
                return response()->json([
                    'status' => 'error',
                    'message' => "You can only change your phone number once every 30 days. You can change again after ".$nextChangeDate
                ]);
            }
        }
